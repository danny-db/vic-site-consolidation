...
spark.sql(f"""
    ...
    -- Guards: hull_efficiency >= 0.6 (flags concave parcels),
    -- hull_quarter_perim² >= hull_area (quadratic discriminant must be non-negative),
    -- and hull_area > 0 (avoid division by zero).
    measurements AS (
        SELECT
            *,
    ...
    SELECT
        ...
        -- Lots in same plan (subdivision size indicator)
        -- High values indicate mass subdivisions (growth-area estates)
        COUNT(*) OVER (PARTITION BY plan_number) AS lots_in_plan
...
# MAGIC
# MAGIC **Note:** Visualizations have been moved to the Databricks App. Uncomment cells below to run inline for debugging.
