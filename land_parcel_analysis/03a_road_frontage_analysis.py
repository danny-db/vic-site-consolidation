...
# MAGIC
# MAGIC Compute road frontage features for all parcels using Photon-optimized
# MAGIC spatial join. Each road is buffered by 2m (once via CTE), then
# MAGIC `ST_Intersects` between parcel boundaries and road buffers leverages
# MAGIC Photon's built-in spatial indexing. The `ST_Intersection` + `ST_Length`
# MAGIC computes the actual frontage length in metres.
...
# MAGIC
# MAGIC Spatial join between parcel boundaries and 2m road buffers. Buffer is
# MAGIC computed once per road segment. `ST_Intersection` is also computed once
# MAGIC and reused for both the length calculation and the noise filter.
...
#
# PERFORMANCE: Relies on Photon's built-in spatial join optimization
# (R-tree / Z-order indexing) for ST_Intersects between parcel boundaries
# and road buffers. The 2m buffer is computed once per road via a CTE.
# ST_Intersection is computed once and reused for length + filter.
#
...
# ============================================================================
# APPROACH: Parcel boundary vs road buffer with Photon-optimized spatial join
# ============================================================================
# H3 pre-filter is effective for polygon-polygon joins (notebook 03) but
# problematic for line geometry (roads): thin buffered strips tessellate
# into many H3 cells losing selectivity, and GEOMETRY columns can't be
# used in DISTINCT.
#
# Instead we rely on Photon's built-in spatial join optimization which
# automatically applies R-tree / Z-order indexing on ST_Intersects.
# The 2m buffer is computed once per road via a CTE.
# ============================================================================
spark.sql(f"""
    ...
    road_buffers AS (
        -- Buffer each road segment once (2m catches near-touches)
        SELECT
            ROAD_NAME,
            CLASS_CODE,
            {road_class_sql} AS road_class,
            ST_SRID(geometry) AS road_srid,
            ST_Buffer(geometry, 2) AS road_buffer
        FROM {catalog_name}.{schema_name}.roads
        WHERE geometry IS NOT NULL
          AND ST_SRID(geometry) > 0
    ),
    intersections AS (
        SELECT
            ...
            r.road_class,
            r.road_srid,
            -- Compute frontage intersection once
            ST_Intersection(p.boundary, r.road_buffer) AS frontage_geom
        FROM parcel_boundaries p
        JOIN road_buffers r
            ON ST_Intersects(p.boundary, r.road_buffer)
    )
    SELECT
        parcel_id,
        zone_code,
        ...
        CLASS_CODE,
        road_class,
        ROUND(ST_Length(ST_SetSRID(frontage_geom, road_srid)), 2) AS frontage_length_m
    FROM intersections
    WHERE ST_Length(frontage_geom) > 0.5  -- Filter noise
""")
