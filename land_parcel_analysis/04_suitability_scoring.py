...
# NOTE: This dictionary documents the weights used in the SQL scoring queries
# below. Keep in sync with the CASE statements in the scoring SQL.
scoring_weights = {
    # Opportunity factors (positive scores)
    "hctz_zone": 30,                    # In HCTZ/MUZ zone
    "grz_zone": 15,                     # In GRZ/NRZ zone
    "within_800m_train_tram": 20,       # Within 800m of train/tram
    "within_400m_bus": 10,              # Within 400m of bus
    "within_800m_ac": 15,              # Within 800m of activity centre
    "within_800m_mac": 10,             # Within 800m of metro activity centre
    "below_min_area_500": 25,           # Below 500sqm minimum
    "below_min_area_300": 10,           # Below 300sqm (additive with 500sqm)
    "below_frontage_15m": 20,           # Below 15m frontage
    "narrow_lot": 10,                   # Narrow lot (<10m)
...
# MAGIC %md
# MAGIC ## 5a. Pre-compute Overlay Flags
# MAGIC
# MAGIC The overlays table contains **all** overlay types across Victoria. We only need 5 types
# MAGIC (HO, LSIO, SBO, ESO, VPO). Pre-filtering before the spatial join dramatically reduces
# MAGIC the number of geometry comparisons, then we materialize the result so the scoring query
# MAGIC only needs a simple equi-join on `parcel_id`.
...
# ============================================================================
# OVERLAY FLAG PRE-COMPUTATION
# ============================================================================
# OPTIMIZATION: The full overlays table has many overlay types (DDO, DPO, BMO,
# EMO, etc.) that are irrelevant for suitability scoring. By filtering to only
# the 5 constraint overlay types BEFORE the spatial join, we reduce the number
# of ST_Intersects comparisons by an order of magnitude.
#
# The filtered overlays are joined against parcels using ST_Intersects, and the
# result is materialized as a table so the scoring query avoids any spatial ops.
#
# OVERLAY TYPES:
...
# ============================================================================
spark.sql(f"""
    CREATE OR REPLACE TABLE {catalog_name}.{schema_name}.parcel_overlay_flags AS
    WITH constraint_overlays AS (
        -- Pre-filter to only the 5 overlay types used in scoring
        SELECT ZONE_CODE, geometry
        FROM {catalog_name}.{schema_name}.overlays
        WHERE ZONE_CODE LIKE 'HO%'
           OR ZONE_CODE LIKE 'LSIO%'
           OR ZONE_CODE LIKE 'SBO%'
           OR ZONE_CODE LIKE 'ESO%'
           OR ZONE_CODE LIKE 'VPO%'
    )
    SELECT
        e.parcel_id,
    ...
    LEFT JOIN constraint_overlays o
        ON ST_Intersects(e.geometry, o.geometry)
    GROUP BY e.parcel_id
""")

overlay_flag_count = spark.sql(f"SELECT COUNT(*) FROM {catalog_name}.{schema_name}.parcel_overlay_flags").collect()[0][0]
affected = spark.sql(f"""
    SELECT
        SUM(has_heritage_overlay) AS heritage,
        SUM(has_flood_overlay) AS flood,
        SUM(has_eso_overlay) AS eso,
        SUM(has_vpo_overlay) AS vpo
    FROM {catalog_name}.{schema_name}.parcel_overlay_flags
""").collect()[0]
print(f"Created parcel_overlay_flags: {overlay_flag_count:,} parcels")
print(f"  Heritage: {affected.heritage:,} | Flood: {affected.flood:,} | ESO: {affected.eso:,} | VPO: {affected.vpo:,}")

# COMMAND ----------

# MAGIC %md
# MAGIC ## 5b. Calculate Suitability Scores
...
# - Pre-computed overlay flags (from step 5a above)
#
# All joins are equi-joins on parcel_id — no spatial operations needed here.
#
...
spark.sql(f"""
    ...
    LEFT JOIN {catalog_name}.{schema_name}.parcel_overlay_flags ov ON e.parcel_id = ov.parcel_id
""")
